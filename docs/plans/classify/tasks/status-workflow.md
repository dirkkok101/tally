...
- Rule status identifies the active RuleSetVersion and draft, active, retired, and superseded rule versions with rule version ID, Rule Set Version ID, lifecycle state, actor ID, reason code, created-at timestamp, and predecessor or successor IDs.
- Validation and evaluation status returns retained fingerprints, lifecycle state, aggregate outcome counts, staleness state, and exactly one next-operation enum value without corpus or projection reread.
- Apply status returns authorized fingerprint, terminal item totals, exact unresolved frontier, mutation-possibility state, and exactly one next-operation enum value of retry, resume, or none.
- Feedback, abandonment, and cleanup status returns requested subject ID, event state, actor ID, reason code, aggregate counts, and exactly one next-operation enum value without payload search.
- Unknown subjects return stable not-found; expected failures return exactly one next-operation enum value from retry, resume, re-evaluate, correct, abandon, cleanup, or none as allowed by durable state.
...
- Status is a bounded read-only projection over durable CLASSIFY state and these public identifier types only: subject ID, actor ID, rule version ID, Rule Set Version ID, run ID, transaction ID, fingerprint, result code, and error code.

### Notes

- Every failure-state result contains exactly one next-operation enum value and explicit mutation-possibility metadata.
