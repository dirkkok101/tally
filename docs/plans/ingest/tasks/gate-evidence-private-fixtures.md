...
| `after` | `TALLY_INGEST_PRIVATE_FIXTURE_MANIFEST="$PWD/docs/statements/.ingest-fixture-manifest.json" dotnet test tests/Tally.Tests/Tally.Tests.csproj --filter FullyQualifiedName~Authorized_private_fixture_set_is_deterministic_and_bounded_when_injected --no-restore --verbosity quiet` | exit 0 with exactly 1 discovered test and 0 failures; the in-process loader proves strict repository-relative containment, regular non-symlink source files, owner-only directory/file modes, digests, three fixtures, and two variants without placing private source locators in subprocess arguments | `true` | 300 |
