...
public sealed class OperationRegistry
{
    ...
    private static OperationDescriptor PaymentAttributionDescriptor(string operationId, JsonTypeInfo request, string target) => new(
        operationId, "tally " + operationId.Replace('.', ' '), "mutation", true, request, LedgerJsonContext.Default.PaymentAttributionResult,
        "PaymentAttributionOperationModule." + target,
        (services, _) => services.PaymentAttributions is { } module ? new PaymentAttributionOperationHandler(module, operationId) : new FoundationOperationHandler(),
        "tally " + operationId.Replace('.', ' ') + " --input -", PaymentAttributionErrorsFor(operationId));

    private static IReadOnlyList<ErrorSchema> PaymentAttributionErrorsFor(string operationId) =>
    [
        new(PaymentAttributionPolicy.InvalidError, "validation", 3),
        new(TransactionErrors.NotFound, "not_found", 4),
        new(PaymentIdentityErrors.InstrumentNotFound, "not_found", 4),
        new(PaymentIdentityErrors.CardholderNotFound, "not_found", 4),
        new(PaymentAttributionErrors.TransactionInactive, "lifecycle", 6),
        new(PaymentIdentityErrors.InstrumentArchived, "lifecycle", 6),
        new(PaymentIdentityErrors.CardholderArchived, "lifecycle", 6),
        new(PaymentIdentityErrors.InstrumentAccountNotActive, "lifecycle", 6),
        new(PaymentAttributionErrors.AccountIncompatible, "lifecycle", 6),
