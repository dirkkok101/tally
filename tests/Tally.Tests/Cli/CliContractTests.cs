...
public sealed class CliContractTests
{
    ...
    [Fact] public void TC_LEDGER_STATEMENT_SCOPE_REGISTRATION_registry_contains_exactly_74_provider_neutral_operations() => Assert.Equal(100, OperationRegistry.Create().Descriptors.Count);
    // TC-LEDGER-CONTRACT-DISCOVERY-CONTRACT
    [Fact]
    public void Schema_list_has_a_stable_independent_contract_inventory()
    {
        ...
        Assert.Equal(100, operationIds.Distinct(StringComparer.Ordinal).Count());
        Assert.Equal("budget.insights.evidence.get", operationIds[0]);
